...
- **Symlink:** `~/.config/hypr/display-profiles` → repo profiles (`./install.sh --only displays` or `apply-ml4w-patches.sh`)
- **Runtime layout:** `~/.config/hypr/monitors.conf` (generated by `apply-display-profile.sh`; not symlinked)
- **Config:** `endeavour/displays/config.env`
- **Save after nwg-displays:** `./endeavour/displays/save-display-profile.sh work|home|laptop`
